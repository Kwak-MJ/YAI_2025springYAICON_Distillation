...
def test(model, model_name=None):
    ...
    test_loader = LoadData(train=False, transform=transform, batch_size=256).get_data()
    ...
    print(f"Test Loss: {total_loss/len(test_loader.dataset):.8f}, Top-1 test Accuracy: {top1_acc:.8f}%, Top-5 test Accuracy: {top5_acc:.8f}%")
    print("Test complete")
    print("================================================")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_job', type=str, default='teacher', help='teacher, student, distillation')
    ...
    if MODEL_JOB == 'teacher':
        model = resnet50()
        MODEL_NAME = "resnet50"
        model.load_state_dict(torch.load(f"{ckp_path}/{MODEL_NAME}.pth"))
        ...
        model = modelWithFeat(resnet18(), isStudent=True)
    ...
    elif MODEL_JOB == 'distillation_KYN':
        model = modelWithFeat(resnet18(), isStudent=True)
        ...
        raise ValueError("MODEL_JOB can be 'teacher','student','distillation', 'distillation_KMJ1', or 'distillation_KMJ2' only.")
